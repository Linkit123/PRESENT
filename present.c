...
int main( int argc, char ** const argv )
{
	...
	if ( Opt.Verbose != 0 )
	{
		...
		printf( " to encrypt and decrypt single blocks with the PRESENT-Cipher\n" );
		printf( "---------------------------------------\n\n" );
	}
...
//	{
		uint64_t *subkey;
		uint64_t a = 0;
		Opt.Text ="0113011113213201";
		Opt.Mode=Encrypt_Mode;
		if ( Opt.Mode == Decrypt_Mode )
		{
			// Put out Values
			if ( Opt.Verbose != 0 )
			{
				...
				printf( "Plaintext %# and int: %#"PRIx64" \n",Opt);
				printf( "Plaintext %# and text: %#"PRIx64" \n",Opt.Text);
				printf( "Plaintext %16 + PRIx64 and text: %16"PRIx64" \n",Opt.Text);
		...
		{
			// Put out Values
			Opt.Text="53c98a7a527d7965";
			if ( Opt.Verbose != 0 )
			{
				printf( "Starting values\n" );
				printf( "Ciphertext: %016"PRIx64" \n", Opt.Text);
				printf( "Ciphertext int: %016"PRIx64" \n", Opt);
				if (Opt.KeySize80) printf( "Given Key (80bit): %016"PRIx64" %04"PRIx64"\n\n", Opt.KeyHigh, (Opt.KeyLow&0xFFFF) );
				else printf( "Given Key (128bit): %016"PRIx64" %016"PRIx64"\n\n", Opt.KeyHigh, Opt.KeyLow );
			}
